...
query = sqlalchemy.text("SELECT * FROM [dbo].[garminactivities]")
data_df = pd.read_sql(query, connection)
...
if query:
    ...
    if isinstance(answer, pd.DataFrame):
        st.session_state["last_answer"] = {"type": "dataframe", "value": answer}
    elif isinstance(answer, ChartResponse):
        st.session_state["last_answer"] = {"type": "image", "value": answer.value}
    elif isinstance(answer, str) and os.path.isfile(answer.strip()):
        st.session_state["last_answer"] = {"type": "image", "value": answer.strip()}
    else:
        st.session_state["last_answer"] = {"type": "text", "value": answer}
